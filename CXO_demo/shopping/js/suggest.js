 ...
 ***********************************************/
//function get_data(){}
var datas='about,along,a boy,a man,a music,array,alist,a line,boy,beatiful,beast,best,bell,bring,bill,buy,bye,bb,boys,be fine,city,crazy,car,cell,char,charing,data,daily,duty,dd,dbout,default,eivl,effect,enjoy,english,fill,full,fea';
function get_data(orgina_data){
	var theDatas=new Array();
	for(var i=0,len=orgina_data.split(',').length;i<len;i++){
		theDatas.push({val:1,activity:orgina_data.split(',')[i]});
		};
	return theDatas;
	}
/***********************************************
...
function suggest(keywords,key)
{
	...
	if(keywords != "")
	{
		var terms = get_data(datas);; // sort? -- data should be alphabetical for best results

//var terms =[{val:1,activity:"a"},{val:1,activity:"ab"}];
...
function hideSuggest()
{
	...
	$(function(){	   
	   $("#s").focus(function(){
			$(this).select();	
			if(this.value=='Please enter keyword!') { this.value=''; this.style.color='#333333'; }
							  }); 
	   });
