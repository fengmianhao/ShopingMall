...
(function($) {
	...
	function tree_component () {
		...
		return {
			...
			parseJSON : function (data) {
				...
				if(this.settings.languages.length) {
					for(var i = 0; i < this.settings.languages.length; i++) {
						...
						//code start ==== add by shuosuo 
						if(data.data[this.settings.languages[i]] && data.data[this.settings.languages[i]].cicon && this.settings.theme_name != "themeroller") {
							var cicn = data.data[this.settings.languages[i]].cicon;
							attr["class"] += " " + cicn;
						}
						//code end ==== add by shuosuo 
						str += "<a";
						for(var j in attr) {
					...
					//code start ==== add by shuosuo 
					if(data.data.cicon && this.settings.ui.theme_name != "themeroller") {
						var cicn = data.data.cicon;
						attr["class"] += " " + cicn;
					}
					//code end ==== add by shuosuo 
					str += "<a";
					...
					str += ( (typeof data.data.title).toLowerCase() != "undefined" ? data.data.title : data.data ) + "</p></a>";
				}
			...
			toggle_branch : function (obj) {  //Node展开关闭切换
				...
				// 20090916-chengjuan set tree height
				if(obj.hasClass("closed"))
				{
					var rst = this.open_branch(obj);
					setLeftHeight();
					return rst;
				}
				if(obj.hasClass("open"))
				{
					var rst = this.close_branch(obj);
					setLeftHeight();
					return rst;
				}
				// 20090916-chengjuan set tree height finish
			},
			...
			rename : function (obj) {
				...
				last_value = obj.childNodes[0].innerHTML;
				...
				_this.inp.blur(function(event) {
						...
						$(obj).html( "<p>"+$(obj).parent().find("input").eq(0).attr("value")+"</p>" ).get(0).style.display = ""; 
